...
class Frontier(object):
    def __init__(self, config, restart):
        ...
        if not os.path.exists(self.config.save_file) and not restart:
            ...
            self.logger.info(f"Found existing save file {self.config.save_file}, deleting it.")
            os.remove(self.config.save_file)
        # Load existing save file, or create one if it does not exist.
        self.save = shelve.open(self.config.save_file)
        if restart:
            self.add_url(self.config.seed_url)
        else:
    ...
    def _parse_save_file(self):
        ''' This function can be overridden for alternate file saving techniques. '''
    ...
    def get_tbd_url(self):
        try:
            return self.to_be_downloaded.get(timeout=60)
        except Empty:
            return None
